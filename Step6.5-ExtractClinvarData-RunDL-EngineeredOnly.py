...
class ClinVarDLFoldPipeline:
    def __init__(self, data_dir="Clinvar_Dataset8", results_dir="Clinvar_Dataset8/DeepLearning"):
            ...
            if summary_results['overall_stats']['avg_metrics_by_model']:
                ...
                for model_name, model_stats in summary_results['overall_stats']['avg_metrics_by_model'].items():
                    ...
                    if 'mcc' in mean_metrics:
                        f.write(f"  MCC:       {mean_metrics['mcc']:.4f} (Â±{std_metrics.get('mcc', 0):.4f})\n")
                    if 'auc_roc' in mean_metrics:
                        f.write(f"  AUC:       {mean_metrics['auc_roc']:.4f} (Â±{std_metrics.get('auc_roc', 0):.4f})\n")
                    if 'accuracy' in mean_metrics:
                        f.write(f"  Accuracy:  {mean_metrics['accuracy']:.4f} (Â±{std_metrics.get('accuracy', 0):.4f})\n")
                    if 'f1' in mean_metrics:
                        f.write(f"  F1 Score:  {mean_metrics['f1']:.4f} (Â±{std_metrics.get('f1', 0):.4f})\n")
