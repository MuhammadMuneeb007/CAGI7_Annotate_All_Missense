...
def save_model_and_features(model, feature_names, label_encoder, X_shape, y_shape, 
                           class_names, train_metrics, test_metrics, args_dict):
    """Save the trained model, features, and metadata"""
    
    # Create output directory
    output_dir = Path("Final_Dataset/Features")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"\nSaving model and features to {output_dir}...")
    
    # Save the trained XGBoost model
    model_path = output_dir / "xgboost_clinvar_model.pkl"
    ...
    # Save features as JSON for easier loading
    features_json_path = output_dir / "feature_names.json"
    ...
    metadata = {
        ...
        'preprocessing': {
            'remove_missing': args_dict.get('remove_missing', True),
            'replace_nan_value': args_dict.get('replace_nan', 0),
            'remove_correlated_threshold': args_dict.get('remove_correlated', None),
            'used_smote': True,
            'scaling_method': None  # We didn't use scaling in this model
        },
        'performance': {
        ...
        'training_info': {
            ...
            'script_version': 'Step6.19-FeatureAnalysis-FinalModel1Dataset5.py',
...
def main():
    ...
    # Store args for metadata
    args_dict = {
        'remove_missing': remove_missing,
        'replace_nan': replace_nan_value,
        'remove_correlated': remove_correlated_threshold
    }
    ...
    print(f"✓ Model: Final_Dataset/Features/xgboost_clinvar_model.pkl")
    print(f"✓ Features: Final_Dataset/Features/feature_names.json")
    print(f"✓ Label Encoder: Final_Dataset/Features/label_encoder.pkl")
    print(f"✓ Metadata: Final_Dataset/Features/model_metadata.json")
    print(f"✓ Feature Importance: Final_Dataset/Features/feature_importance.csv")
